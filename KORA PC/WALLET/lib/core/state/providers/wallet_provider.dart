...
import 'package:kora_windows/core/repositories/wallet_repository.dart';
import 'package:kora_windows/core/services/wallet_initialization_service.dart';
import 'package:kora_windows/core/services/token_validation_service.dart';
import 'package:kora_windows/core/crypto/key_manager.dart';
import 'package:kora_windows/core/constants/token_catalog.dart';
import 'package:kora_windows/core/repositories/price_repository.dart' show PriceRepository;
import 'package:kora_windows/core/services/balance_service.dart';
import 'package:kora_windows/core/services/lock_service.dart';
...
class CurrentWalletNotifier extends StateNotifier<AsyncValue<Wallet?>>
  ...
  final Map<String, Timer> _optimisticRefreshTimers = {};
  // assetId → optimistic balance string; guards against stale-cache overwrite
  final Map<String, String> _optimisticBalances = {};

  /// Publishes a wallet, but only when it differs from the one already published.
  ///
  /// `Wallet` and `Asset` are freezed, so `!=` is a real comparison of every field rather than
  /// an identity check. That matters more than it looks: a StateNotifier emits on every
  /// assignment, and every emission hands each downstream provider a freshly-built List, which
  /// Riverpod compares by identity and therefore always treats as changed. One assignment
  /// rebuilt every screen, re-ran the portfolio curve and re-fired the whole history fan-out
  /// over the network.
  ///
  /// The three-minute balance poll and the hourly price poll almost always come back with
  /// exactly what is already on screen. Those are now silent.
  void _publish(Wallet? wallet) {
    if (wallet == null) return;
    if (state.valueOrNull == wallet) return;
  ...
  /// True while the app is showing its lock screen.
  ///
  /// Nothing behind the lock screen is on show, so nothing behind it needs to be current.
  /// Polling through a lock spends the user's battery and their rate limit on figures no one
  /// can see, and announces to every explorer that the wallet exists on a fixed schedule for
  /// as long as the machine is awake. The refresh on unlock brings everything up to date in
  /// one pass anyway.
  bool get _locked => _ref.read(lockProvider);
  ...
  void _startPriceRefreshTimer() {
    ...
    _priceRefreshTimer = Timer.periodic(const Duration(hours: 1), (_) async {
      final wallet = state.valueOrNull;
      if (wallet == null || !mounted || _locked) return;
      try {
        final priceSvc = WalletInitializationService();
        final updated  = await priceSvc.updateAssetPrices(wallet.assets);
        final saved = await _repository.updateWalletAssets(wallet.id, updated);
        if (mounted) _publish(saved);
  ...
  Future<void> _deriveAndAddRemainingAssets(String walletId, String mnemonic) async {
    ...
    try {
      ...
      if (mounted) _publish(await _repository.getCurrentWallet());
  ...
  Future<void> refreshBalances() async {
    ...
    // `StackTrace.current` used to be printed here on every refresh. Capturing a stack trace
    // is not free, and this runs on a three-minute timer as well as after every send.
    if (kDebugMode) debugPrint('[WalletProvider] Refreshing balances…');
    try {
  ...
  Future<void> _fetchAndStoreBalances(String walletId, List<Asset> assets, int gen) async {
    ...
    try {
      ...
      // The generation is checked BEFORE the write, not only before the publish. A fetch that
      // started against the previous wallet — or before an optimistic balance was applied —
      // was still allowed to write its stale numbers to disk; only the on-screen state was
      // protected. The next launch then read back the balances that had been superseded.
      if (!mounted || _fetchGeneration != gen) {
        if (kDebugMode) debugPrint('[WalletProvider] gen=$gen stale before write — discarded');
        return;
      }
      final saved = await _repository.updateWalletAssets(walletId, updated);
      sw.stop();
  ...
  Future<void> switchWallet(String walletId) async {
    _forgetOptimisticBalances();
    await _repository.setCurrentWallet(walletId);
    await _loadWallet();
  }

  /// Drops the balances written ahead of the chain by the open wallet's sends.
  ///
  /// They are keyed by asset id, and asset ids repeat across wallets — every wallet has a
  /// `bitcoin`. Carried into the next wallet, an entry would clamp that wallet's real balance
  /// to a number from a different wallet, and `refreshBalances` writes the clamped figure to
  /// disk. The wrong balance would outlive the session.
  void _forgetOptimisticBalances() {
    if (_optimisticBalances.isEmpty && _optimisticRefreshTimers.isEmpty) return;
    for (final timer in _optimisticRefreshTimers.values) timer.cancel();
    _optimisticRefreshTimers.clear();
    _optimisticBalances.clear();
  }
  ...
  Future<void> applyOptimisticBalance(
    ...
    final updatedAssets = wallet.assets.map((a) {
      ...
      // The USD figure has to move with the quantity. Writing only `balance` left
      // `balanceInUsd` at its pre-send value, so after sending, the coin row showed the new
      // amount beside the old worth and the portfolio total did not change at all — which
      // reads as "the transaction did nothing".
      final quantity = double.tryParse(newBalance) ?? 0;
      return a.copyWith(balance: newBalance, balanceInUsd: quantity * a.priceUsd);
    }).toList();
    ...
    // One timer per asset, not one shared timer.
    //
    // Sending from a second coin inside the window used to cancel the first coin's cleanup,
    // and the replacement closure only ever removed its own asset — so the first entry was
    // orphaned and went on clamping that coin's real balance until it happened to fall.
    _optimisticRefreshTimers.remove(assetId)?.cancel();
    _optimisticRefreshTimers[assetId] = Timer(refreshDelay, () {
      _optimisticRefreshTimers.remove(assetId);
      if (mounted) {
