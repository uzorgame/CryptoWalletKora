...
class EthereumWallet {
  ...
  factory EthereumWallet.fromMnemonic(String mnemonic, {int index = 0, String blockchain = 'ethereum'}) =>
      EthereumWallet.fromHdWallet(HDWallet.fromMnemonic(mnemonic),
          index: index, blockchain: blockchain);

  /// From an already-derived HD wallet.
  ///
  /// The seed behind it costs 2048 rounds of PBKDF2, so the send path derives it once in an
  /// isolate (see `core/crypto/seed.dart`) and builds from the result rather than paying for
  /// it again on the interface thread.
  factory EthereumWallet.fromHdWallet(HDWallet hdWallet,
      {int index = 0, String blockchain = 'ethereum'}) {
