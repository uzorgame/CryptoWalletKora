...
class EthereumClassicFeeNotifier extends StateNotifier<AsyncValue<FeeEstimate?>> {
  ...
  Future<void> fetchFee({bool forceRefresh = false}) async {
    ...
    try {
      if (!forceRefresh) {
        final cached = await FeeCacheService.getCachedFee(blockchain: 'ethereum_classic', speed: speed);
  ...
  Future<FeeEstimate> _addUsd(FeeEstimate fee) async {
    try {
      ...
      return fee.copyWith(feeInUsd: fee.feeInNative * (prices['ethereum-classic']?.priceUsd ?? 0.0));
    } catch (_) { return fee; }
  }
}
