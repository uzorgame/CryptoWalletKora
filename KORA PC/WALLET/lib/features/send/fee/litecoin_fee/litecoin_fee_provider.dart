...
class LitecoinFeeNotifier extends StateNotifier<AsyncValue<FeeEstimate?>> {
  ...
  Future<void> fetchFee({bool forceRefresh = false}) async {
    ...
    try {
      if (!forceRefresh) {
        final cached = await FeeCacheService.getCachedFee(blockchain: 'litecoin', speed: speed);
  ...
  Future<FeeEstimate> _addUsd(FeeEstimate fee) async {
    try {
      ...
      return fee.copyWith(feeInUsd: fee.feeInNative * (prices['litecoin']?.priceUsd ?? 0.0));
    } catch (_) { return fee; }
  }
}
